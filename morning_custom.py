...
if __name__ == "__main__":
    # 1) Today's temperature (Seattle)
    wx = fetch_today_high_low(CITY)
    high_f, low_f = c_to_f(wx["high_c"]), c_to_f(wx["low_c"])

    # 2) Top 5 stories about Seattle
    seattle_5 = seattle_top(SEATTLE_LIMIT)

    # 3) Top 3 world stories
    world_3 = newsapi_top_world(WORLD_LIMIT)

    # 4) Banking summary
    banking = ai_summarize_banking()

    # Console view
    today = datetime.date.today().strftime("%Y-%m-%d")
    print(f"\n=== Morning Brief ({today}) ===")
    print(
        f"{wx['city']} Temps Today: High {high_f:.1f}°F / Low {low_f:.1f}°F  "
        f"(High {wx['high_c']:.1f}°C / Low {wx['low_c']:.1f}°C)"
    )
    ...
    <h3>Top {len(seattle_5)} Seattle Stories</h3>
    <ol>{list_items(seattle_5)}</ol>

    <h3>Top {len(world_3)} World Stories</h3>
    <ol>{list_items(world_3)}</ol>

    <h3>Banking Summary</h3>
    <p>{banking['summary'].replace('\n', '<br/>')}</p>
    """

    try:
        send_email(subject=f"Morning Brief — {wx['city']} & World ({today})", html_body=html)
        print("📧 Email sent.")
    ...
    if SAVE_TO_DISK:
        ...
        with open(f"morning_{stamp}.json", "w", encoding="utf-8") as f:
            json.dump(
                {"weather": wx, "seattle": seattle_5, "world": world_3, "banking": banking},
                f,
                indent=2,
            )
        print("💾 Saved:", f.name)
